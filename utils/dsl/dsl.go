...
func ParseCrossNodeDependencies(def types.Chain) map[string][]string {
...
func GetReferencedNodeIds(def types.Chain) []string {
...
func IsNodeIdDefined(def types.Chain, nodeId string) bool {
	for _, node := range def.Metadata.Nodes {
		if node.Id == nodeId {
			return true
		}
...
// ParseVars 解析规则链中的变量
func ParseVars(varPrefix string, def types.Chain, includeNodeId ...string) []string {
...
func IsFlowNode(def types.Chain, nodeId string) bool {
	for _, node := range def.Metadata.Nodes {
...
func ProcessVariables(config types.Config, ruleChainDef types.Chain, from types.Configuration) types.Configuration {
...
func GetInitNodeEnv(config types.Config, ruleChainDef types.Chain) map[string]any {
	varsEnv := ruleChainDef.Configuration[types.Vars]
	globalEnv := make(map[string]any)
